...
var color_scale3 = d3.scale.quantile()
    ...
    .range(colorbrewer.Greens[10]);
var chart_transform_x=596,chart_transform_y=380;
var subunit_label_offset_x=40,subunit_label_offset_y=80
var colorbrewer = {
    Greys: {
...
function ready( error, world, match_id_to_name, country_info ) {
    ...
    for (var ii = 0; ii < match_id_to_name.length; ii++) {
        ...
        match_name_to_id_[match_id_to_name[ii].name] = {id: match_id_to_name[ii].id}; //make new associated array
    }
    ...
    for (i=0;i<unsorted_countries.length;i++)
    {
        ...
        sorted_countries[lookup_] = {
            id: lookup_,
            name: match_id_to_name_[lookup_].name,
...
//<editor-fold defaultstate="collapsed" desc="WORLD">
    ...
    g= svg.selectAll(".country")
        ...
        .attr("transform", "translate( "+-subunit_label_offset_x+" ,"+-subunit_label_offset_y+")")
        .attr("class", "country")
        .attr("id", function(d,i) { return ("country_id"+[d.id]) } )
        .attr("d", path)
        .style("fill", "#FFFFFF")
    g.transition()
    ...
    t=svg.selectAll(".subunit-label")
        ...
        .attr("transform", function(d,i) { var x_=path.centroid(d)[0]-subunit_label_offset_x;var y_=path.centroid(d)[1]-subunit_label_offset_y;add_coord_sorted_country(sorted_countries[d.id].name,x_,y_); return "translate("+x_+","+y_+")";})
        .attr("dy", ".35em")
        .style("pointer-events", "none")
        .style("display", "none")
...
//</editor-fold>
    function click(d) {
        ...
        if (d && centered !== d) {
            ...
            d3.selectAll(".subunit-label") .style("display", "none").attr("transform", function(d) {var x_=path.centroid(d)[0]-subunit_label_offset_x;var y_=path.centroid(d)[1]-subunit_label_offset_y; return "translate("+x_+","+y_+")"; })
            svg.select("#label_under_text") .style("display", "none")
    ...
    window.ii=1;
   // redraw( sorted_countries, sorted_values);
...
function draw_chart (sorted_values){
    ...
    for (i=0;i<10;i++) { //top 10
        array_for_chart[i] = {
            name: sorted_values[i].name,
            value1: +sorted_values[i][field_name]
        }
    }
    ...
    var arcs = svg.selectAll(".arc")
        ...
        .enter().append("g")
        .attr("class", "arc")
        .attr("transform", "translate("+chart_transform_x+","+chart_transform_y+")");

     var paths=arcs.append("path")
        .style("fill", function(d,i) { var coord=arc.centroid(d);create_lines_array(i,d.data.name,coord); return color(d.data.value1); })
    .transition()
        .duration(2000)
    ...
    .transition()
      ...
      arcs.append("text")
        .attr("transform", function(d) {  var coord=arc.centroid(d); return "translate(" + arc.centroid(d) + ")"; })
    ...
    svg.selectAll(".gg").remove()
    var gg = svg.append("g")
        .attr("class", "gg")
        .attr("transform", "translate("+chart_transform_x+","+chart_transform_y+")")

    svg.selectAll(".lineGraph").remove()
    var lineGraph = gg.selectAll(".lineGraph")
        .data(lineData)
        .enter().append("path")
...
function create_lines_array (i,country_name,chart_coord)
{

    // var country_coord=[500,500];
    // var c=svg.select("#"+country_name)
    // tt=c.getAttribute("transform");
    // .attr("transform", function(d,i) { console.log(path.centroid(d));country_coord=path.centroid(d);})

    var country_id=match_name_to_id_[country_name].id

    svg.select("#country_id"+country_id).each( function(d, i){
      //  if(d.someId == targetId){
            country_coord=path.centroid(d);
      //  console.log(country_coord);
      //  }
    })

   //lineData[i] = [ { "x": country_coord[0]-chart_transform_x-subunit_label_offset_x,  "y": country_coord[1]-chart_transform_y-subunit_label_offset_y }, { "x": chart_coord[0]+300,  "y": chart_coord[1]+300  }, { "x": chart_coord[0],  "y": chart_coord[1] } ];
   lineData[i] = [ { "x": country_coord[0]-chart_transform_x-subunit_label_offset_x,  "y": country_coord[1]-chart_transform_y-subunit_label_offset_y }, { "x": 0,  "y": -340 }, { "x": chart_coord[0],  "y": chart_coord[1] } ];
}

//!Delete IT
function add_coord_sorted_country (country_name,x_,y_)
{
    sorted_countries[country_name] = {   x: x_,y: y_ };
   // console.log(sorted_countries[country_name]);
}
