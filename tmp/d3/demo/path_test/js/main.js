...
function ready( error, world, match_id_to_name, country_info ) {
    ...
    g.transition()
        .style("fill", function(d,i) { return d3.rgb(color_scale(sorted_countries[d.id].value1)); })
    ...
    g.on("mouseout", function(d) {
        d3.select( this ).style("fill", function(d) {return d3.rgb(color_scale(sorted_countries[d.id].value1)); }) ,
    ...
    var ii=1;
  var IntervalId=setInterval(function(){ii++; if (ii>3){ii=1};redraw( ii, sorted_countries);}, 10000)
...
function redraw (ii, sorted_countries){
    var color_scale = d3.scale.quantile()
        .domain([0, 1, 1000, 121939, 903449])
        .range(colorbrewer.Greens[10]);
...
/*    g.remove();
    ...
    g= svg.selectAll(".country")
      g.transition()
     .style("fill", function(d,i) { return d3.rgb(color_scale(sorted_countries[d.id][field_name])); })
          //.style("fill", function(d,i) { console.log('sorted_countries['+d.id+'].'+field_name);console.log(d3.rgb('sorted_countries['+d.id+'].'+field_name)); return d3.rgb(color_scale('sorted_countries['+d.id+'].'+field_name)); })
     .delay(function(d, i) { return 1200+(i * 12); })
