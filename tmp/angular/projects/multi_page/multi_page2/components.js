...
app.controller ("AppCtrl", function ($scope, $http ) {
    ...
    function readJson_menu ($scope, $http,jsonFileName ) {
        $http.get(jsonFileName).success(function (data) {
            $scope.items_menu = data;
        })
    }
    ...
    readJson_menu ($scope, $http,'menu.json');
    readJson_news ($scope, $http,'news.json');


    $scope.singlePageView=function (selected_id) {
        $scope.selected_item="page";
        $scope.selected_id=selected_id-1;
        //console.log(">1", $scope.jsonFileName);
    }
