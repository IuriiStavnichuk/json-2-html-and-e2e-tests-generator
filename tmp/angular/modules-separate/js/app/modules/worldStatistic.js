...
queue()
    .defer(d3.json, "json/world-statistic/world-110m.json")
    .defer(d3.tsv, "json/world-statistic/world-country-names.tsv")  // match id to country name
    .defer(d3.csv, "json/world-statistic/country.csv") // info about country
    .await(ready);
...
function ready( error, world, match_id_to_name, country_info ) {
    ...
    setTimeout(function(){ draw_chart(sorted_values)},5300)
...
function draw_chart (sorted_values){
    ...
    var sum =0;
    d3.sum(sorted_values, function(d,i) { (typeof(d) != "undefined")?sum=+d[field_name]+sum:0 });

    var array_for_chart = [];

    for (i=0;i<10;i++) { array_for_chart[i] = { name: sorted_values[i].name,  value1: +sorted_values[i][field_name],  percent:  Math.ceil(+sorted_values[i][field_name]/(sum/100))    }; }    //top 10
    ...
    var arcs = svg.selectAll(".arc")
        ...
        .enter().append("g")
        .attr("class", "arc")
        .attr("transform", "translate(600,268)");
    ...
    arcs.append("text")
        ...
        .html(function(d) { return ( d.data.name+" "+d.data.percent+"%" ) })
