        ...
        link: function ($scope) {
            ...
            $scope.resourceInit = function () {
                ...
                switch ($scope.currentLanguage) {
                    case '\"en-US\"':
                        // for Checkout
                        $http.get('../js/app/modules/questionnaireCheckout_resources_en2.json')
                        ...
                        break;
                    case '\"fr-CA\"':
                        $http.get('../js/app/modules/questionnaireCheckout_resources_fr2.json')
                            .success(function (data) {
                                $scope.resource = data;
                                if ($scope.info.urlQuestionnaireTypeId.value === 3) {                           //for Charity
            ...
            $scope.htmlInit = function () {
                ...
                $scope.info.idOfRecord.value = $("#id-of-page").val();

                $("#submitDocTitle+fieldset").append($('#checkout-document-upload-form'));

                $('#checkout-document-upload-form').css("display", "block");

                $('#button-upload-website-review-documents').click(function () {
                    var idOfPage = $('#id-of-page').val();
                    $('#container-id-of-website-review-for-upload input').val(idOfPage);
                    $('.js-unique-button-upload-postback').click();
                    return false;
                });
            ...
            $scope.openSection = function (sectionIdForShowHide) {
                ...
                if (sectionIdForShowHide != "introductionSectionTitle") $('html, body').animate({ scrollTop: 340 }, 'fast');
            ...
            $scope.prepareDataForServer = function () {
                ...
                if (objName) {
                    //if ($scope.info.urlFieldSaleOriginTypeIdList !== null && Object.getOwnPropertyNames($scope.info.urlFieldSaleOriginTypeIdList).length !== 0) {
                    objName.value = [];
                    ...
                    //if (typeof objName.easternEurope.value !== 'undefined' && (objName.easternEurope.value)) { objName.value.push(parseInt(objName.easternEurope.value)); }
            ...
            $scope.saveForm = function (sectionTitle) {
                ...
                //$scope.info.previousUrlsId.value = $scope.info.businessModel.value;
                ...
                $http({
                    ...
                    success(function (response) {
                        ...
                        $("#id-of-page").val($scope.info.idOfRecord.value);
                        $scope.convertDataIntoAngularFormat();
                        $(".loading-indicator-lightbox").css("display", "none");
            ...
            $scope.getWebsiteReviewQuestionnaire = function () {
                ...
                $http.get("../JSON/WebsiteApi/GetWebsiteQuestionnaire/" + $scope.info.idOfRecord.value)
                    .success(function (data) {
                        ...
                        $(".panel-heading .loading-indicator-lightbox").remove();
